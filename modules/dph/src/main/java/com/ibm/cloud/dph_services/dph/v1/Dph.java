...
public class Dph extends BaseService {
   ...
   *
   * @param serviceName the service name to be used when configuring the client
   *                    instance
   * @return an instance of the `Dph` client using external configuration
   */
   ...
   * The specified service name and authenticator are used to configure the client
   * instance.
   *
   * @param serviceName   the service name to be used when configuring the client
   *                      instance
   * @param authenticator the {@link Authenticator} instance to be configured for
   *                      this client
   */
   ...
   * Hub.&lt;br/&gt;&lt;br/&gt;If the data
   * product catalog exists but has never been initialized, the status will be
   * "not_started".&lt;br/&gt;&lt;br/&gt;If
   * the data product catalog exists and has been or is being initialized, the
   * response will contain the status of the
   * last or current initialization. If the initialization failed, the "errors"
   * and "trace" fields will contain the
   * error(s) encountered during the initialization, including the ID to trace the
   * error(s).&lt;br/&gt;&lt;br/&gt;If the
   * data product catalog doesn't exist, an HTTP 404 response is returned.
   *
   * @param getInitializeStatusOptions the {@link GetInitializeStatusOptions}
   *                                   containing the options for the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link InitializeResource}
   */
  public ServiceCall<InitializeResource> getInitializeStatus(GetInitializeStatusOptions getInitializeStatusOptions) {
    ...
    RequestBuilder builder = RequestBuilder
        .get(RequestBuilder.resolveRequestUrl(getServiceUrl(), CommonConstants.URL_GET_INITIALIZE_STATUS));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.GET_INITIALIZE_STATUS);
    ...
    ResponseConverter<InitializeResource> responseConverter = ResponseConverterUtils
        .getValue(new com.google.gson.reflect.TypeToken<InitializeResource>() {
        }.getType());
   ...
   *
   * Use this API to get the information of service id credentials in Data Product
   * Hub.
   *
   * @param getServiceIdCredentialsOptions the
   *                                       {@link GetServiceIdCredentialsOptions}
   *                                       containing the options for the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link ServiceIdCredentials}
   */
  public ServiceCall<ServiceIdCredentials> getServiceIdCredentials(
      GetServiceIdCredentialsOptions getServiceIdCredentialsOptions) {
    RequestBuilder builder = RequestBuilder
        .get(RequestBuilder.resolveRequestUrl(getServiceUrl(), CommonConstants.URL_GET_SERVICEID_CREDENTIALS));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.GET_SERVICEID_CREDENTIALS);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
    ResponseConverter<ServiceIdCredentials> responseConverter = ResponseConverterUtils
        .getValue(new com.google.gson.reflect.TypeToken<ServiceIdCredentials>() {
        }.getType());
    return createServiceCall(builder.build(), responseConverter);
  }
   ...
   * &lt;br/&gt;&lt;br/&gt;You can initialize:
   * &lt;br/&gt;&lt;ul&gt;&lt;li&gt;`delivery_methods` - Methods through which
   * data product parts can be delivered to
   * consumers of the data product
   * hub&lt;/li&gt;&lt;li&gt;`domains_multi_industry` - Taxonomy of domains and
   * use cases
   * applicable to multiple industries&lt;/li&gt;&lt;li&gt;`data_product_samples`
   * - Sample data products used to
   * illustrate capabilities of the data product
   * hub&lt;/li&gt;&lt;li&gt;`workflows` - Workflows to enable restricted
   * data products&lt;/li&gt;&lt;li&gt;`project` - A default project for exporting
   * data assets to
   * files&lt;/li&gt;&lt;li&gt;`catalog_configurations` - Catalog configurations
   * for the default data product
   * catalog&lt;/li&gt;&lt;/ul&gt;&lt;br/&gt;&lt;br/&gt;If a resource depends on
   * resources that are not specified in the
   * request, these dependent resources will be automatically initialized. E.g.,
   * initializing `data_product_samples`
   * will also initialize `domains_multi_industry` and `delivery_methods` even if
   * they are not specified in the request
   * because it depends on them.&lt;br/&gt;&lt;br/&gt;If initializing the data
   * product hub for the first time, do not
   * specify a container. The default data product catalog will be
   * created.&lt;br/&gt;For first time initialization, it
   * is recommended that at least `delivery_methods` and `domains_multi_industry`
   * is included in the initialize
   * operation.&lt;br/&gt;&lt;br/&gt;If the data product hub has already been
   * initialized, you may call this API again
   * to initialize new resources, such as new delivery methods. In this case,
   * specify the default data product catalog
   * container information.
   *
   * @param initializeOptions the {@link InitializeOptions} containing the options
   *                          for the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link InitializeResource}
   */
  public ServiceCall<InitializeResource> initialize(InitializeOptions initializeOptions) {
    ...
    RequestBuilder builder = RequestBuilder
        .post(RequestBuilder.resolveRequestUrl(getServiceUrl(), CommonConstants.URL_INITIALIZE));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.INITIALIZE);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
    ...
    if (initializeOptions.container() != null) {
      contentJson.add(CommonConstants.CONTAINER,
          com.ibm.cloud.sdk.core.util.GsonSingleton.getGson().toJsonTree(initializeOptions.container()));
    }
    if (initializeOptions.include() != null) {
      contentJson.add(CommonConstants.INCLUDE,
          com.ibm.cloud.sdk.core.util.GsonSingleton.getGson().toJsonTree(initializeOptions.include()));
    }
   ...
   *
   * @param manageApiKeysOptions the {@link ManageApiKeysOptions} containing the
   *                             options for the call
   * @return a {@link ServiceCall} with a void result
   */
  public ServiceCall<Void> manageApiKeys(ManageApiKeysOptions manageApiKeysOptions) {
    RequestBuilder builder = RequestBuilder
        .post(RequestBuilder.resolveRequestUrl(getServiceUrl(), CommonConstants.URL_MANAGE_APIKEYS));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.MANAGE_APIKEYS);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
   ...
   *
   * @param listDataProductsOptions the {@link ListDataProductsOptions} containing
   *                                the options for the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link DataProductSummaryCollection}
   */
  public ServiceCall<DataProductSummaryCollection> listDataProducts(ListDataProductsOptions listDataProductsOptions) {
    ...
    RequestBuilder builder = RequestBuilder
        .get(RequestBuilder.resolveRequestUrl(getServiceUrl(), CommonConstants.URL_LIST_DATA_PRODUCTS));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.LIST_DATA_PRODUCTS);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
    ...
    ResponseConverter<DataProductSummaryCollection> responseConverter = ResponseConverterUtils
        .getValue(new com.google.gson.reflect.TypeToken<DataProductSummaryCollection>() {
        }.getType());
    return createServiceCall(builder.build(), responseConverter);
  }
   ...
   *
   * Use this API to create a new data product.&lt;br/&gt;&lt;br/&gt;Provide the
   * initial draft of the data
   * product.&lt;br/&gt;&lt;br/&gt;Required fields:&lt;br/&gt;&lt;br/&gt;-
   * name&lt;br/&gt;-
   * container&lt;br/&gt;&lt;br/&gt;If `version` is not specified, the default
   * version **1.0.0** will be
   * used.&lt;br/&gt;&lt;br/&gt;The `domain` is optional.
   *
   * @param createDataProductOptions the {@link CreateDataProductOptions}
   *                                 containing the options for the call
  ...
  public ServiceCall<DataProduct> createDataProduct(CreateDataProductOptions createDataProductOptions) {
    ...
    RequestBuilder builder = RequestBuilder
        .post(RequestBuilder.resolveRequestUrl(getServiceUrl(), CommonConstants.URL_CREATE_DATA_PRODUCT));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.CREATE_DATA_PRODUCT);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
    final JsonObject contentJson = new JsonObject();
    contentJson.add(CommonConstants.DRAFTS,
        com.ibm.cloud.sdk.core.util.GsonSingleton.getGson().toJsonTree(createDataProductOptions.drafts()));
    builder.bodyJson(contentJson);
    ResponseConverter<DataProduct> responseConverter = ResponseConverterUtils
        .getValue(new com.google.gson.reflect.TypeToken<DataProduct>() {
        }.getType());
    return createServiceCall(builder.build(), responseConverter);
  }
   ...
   *
   * @param getDataProductOptions the {@link GetDataProductOptions} containing the
   *                              options for the call
   * @return a {@link ServiceCall} with a result of type {@link DataProduct}
   */
  public ServiceCall<DataProduct> getDataProduct(GetDataProductOptions getDataProductOptions) {
    ...
    RequestBuilder builder = RequestBuilder
        .get(RequestBuilder.resolveRequestUrl(getServiceUrl(), CommonConstants.URL_GET_DATA_PRODUCT, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.GET_DATA_PRODUCT);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
   ...
   *
   * After uploading a file to the provided signed URL, call this endpoint to mark
   * the upload as complete. After the
   * upload operation is marked as complete, the file is available to download.
   * - After the upload is marked as complete, the returned URL is displayed in
   * the "url" field. The signed URL is used
   ...
   *
   * @param completeDraftContractTermsDocumentOptions the
   *                                                  {@link CompleteDraftContractTermsDocumentOptions}
   *                                                  containing the options for
   *                                                  the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link ContractTermsDocument}
   */
  public ServiceCall<ContractTermsDocument> completeDraftContractTermsDocument(
      CompleteDraftContractTermsDocumentOptions completeDraftContractTermsDocumentOptions) {
    ...
    RequestBuilder builder = RequestBuilder.post(RequestBuilder.resolveRequestUrl(getServiceUrl(),
        CommonConstants.URL_COMPLETE_DRAFT_CONTRACT_TERMS_DOCUMENT, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.COMPLETE_DRAFT_CONTRACT_TERMS_DOCUMENT);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
    ResponseConverter<ContractTermsDocument> responseConverter = ResponseConverterUtils
        .getValue(new com.google.gson.reflect.TypeToken<ContractTermsDocument>() {
        }.getType());
    return createServiceCall(builder.build(), responseConverter);
  }
   ...
   *
   * @param listDataProductDraftsOptions the {@link ListDataProductDraftsOptions}
   *                                     containing the options for the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link DataProductDraftCollection}
   */
  public ServiceCall<DataProductDraftCollection> listDataProductDrafts(
      ListDataProductDraftsOptions listDataProductDraftsOptions) {
    ...
    RequestBuilder builder = RequestBuilder.get(
        RequestBuilder.resolveRequestUrl(getServiceUrl(), CommonConstants.URL_LIST_DATA_PRODUCT_DRAFTS, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.LIST_DATA_PRODUCT_DRAFTS);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
    if (listDataProductDraftsOptions.assetContainerId() != null) {
      builder.query(CommonConstants.ASSET_CONTAINER_ID,
          String.valueOf(listDataProductDraftsOptions.assetContainerId()));
    }
    ...
    ResponseConverter<DataProductDraftCollection> responseConverter = ResponseConverterUtils
        .getValue(new com.google.gson.reflect.TypeToken<DataProductDraftCollection>() {
        }.getType());
    return createServiceCall(builder.build(), responseConverter);
  }
   ...
   *
   * @param createDataProductDraftOptions the
   *                                      {@link CreateDataProductDraftOptions}
   *                                      containing the options for the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link DataProductVersion}
   */
  public ServiceCall<DataProductVersion> createDataProductDraft(
      CreateDataProductDraftOptions createDataProductDraftOptions) {
    ...
    RequestBuilder builder = RequestBuilder.post(RequestBuilder.resolveRequestUrl(getServiceUrl(),
        CommonConstants.URL_CREATE_DATA_PRODUCT_DRAFT, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.CREATE_DATA_PRODUCT_DRAFT);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
    final JsonObject contentJson = new JsonObject();
    contentJson.add(CommonConstants.ASSET,
        com.ibm.cloud.sdk.core.util.GsonSingleton.getGson().toJsonTree(createDataProductDraftOptions.asset()));
    ...
    if (createDataProductDraftOptions.dataProduct() != null) {
      contentJson.add(CommonConstants.DATA_PRODUCT,
          com.ibm.cloud.sdk.core.util.GsonSingleton.getGson().toJsonTree(createDataProductDraftOptions.dataProduct()));
    }
    ...
    if (createDataProductDraftOptions.tags() != null) {
      contentJson.add(CommonConstants.TAGS,
          com.ibm.cloud.sdk.core.util.GsonSingleton.getGson().toJsonTree(createDataProductDraftOptions.tags()));
    }
    if (createDataProductDraftOptions.useCases() != null) {
      contentJson.add(CommonConstants.USE_CASES,
          com.ibm.cloud.sdk.core.util.GsonSingleton.getGson().toJsonTree(createDataProductDraftOptions.useCases()));
    }
    if (createDataProductDraftOptions.types() != null) {
      contentJson.add(CommonConstants.TYPES,
          com.ibm.cloud.sdk.core.util.GsonSingleton.getGson().toJsonTree(createDataProductDraftOptions.types()));
    }
    if (createDataProductDraftOptions.contractTerms() != null) {
      contentJson.add(CommonConstants.CONTRACT_TERMS, com.ibm.cloud.sdk.core.util.GsonSingleton.getGson()
          .toJsonTree(createDataProductDraftOptions.contractTerms()));
    }
    ...
    if (createDataProductDraftOptions.domain() != null) {
      contentJson.add(CommonConstants.DOMAIN,
          com.ibm.cloud.sdk.core.util.GsonSingleton.getGson().toJsonTree(createDataProductDraftOptions.domain()));
    }
    if (createDataProductDraftOptions.partsOut() != null) {
      contentJson.add(CommonConstants.PARTS_OUT,
          com.ibm.cloud.sdk.core.util.GsonSingleton.getGson().toJsonTree(createDataProductDraftOptions.partsOut()));
    }
    if (createDataProductDraftOptions.workflows() != null) {
      contentJson.add(CommonConstants.WORKFLOWS,
          com.ibm.cloud.sdk.core.util.GsonSingleton.getGson().toJsonTree(createDataProductDraftOptions.workflows()));
    }
    builder.bodyJson(contentJson);
    ResponseConverter<DataProductVersion> responseConverter = ResponseConverterUtils
        .getValue(new com.google.gson.reflect.TypeToken<DataProductVersion>() {
        }.getType());
    return createServiceCall(builder.build(), responseConverter);
  }
   ...
   * - If the request object does not contain a "url" parameter, an attachment
   * document is created, and a signed url
   * will be returned in an "upload_url" parameter. The data product producer can
   * upload the document using the provided
   * "upload_url". After the upload is completed, call
   * "complete_contract_terms_document" for the given document needs
   * to be called to mark the upload as completed. After completion of the upload,
   * "get_contract_terms_document" for the
   * given document returns a signed "url" parameter that can be used to download
   * the attachment document.
   *
   * @param createDraftContractTermsDocumentOptions the
   *                                                {@link CreateDraftContractTermsDocumentOptions}
   *                                                containing the options for the
   *                                                call
   * @return a {@link ServiceCall} with a result of type
   *         {@link ContractTermsDocument}
   */
  public ServiceCall<ContractTermsDocument> createDraftContractTermsDocument(
    ...
    RequestBuilder builder = RequestBuilder.post(RequestBuilder.resolveRequestUrl(getServiceUrl(),
        CommonConstants.URL_CREATE_DRAFT_CONTRACT_TERMS_DOCUMENT, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.CREATE_DRAFT_CONTRACT_TERMS_DOCUMENT);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
   ...
   *
   * @param getDataProductDraftOptions the {@link GetDataProductDraftOptions}
   *                                   containing the options for the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link DataProductVersion}
   */
  public ServiceCall<DataProductVersion> getDataProductDraft(GetDataProductDraftOptions getDataProductDraftOptions) {
    ...
    RequestBuilder builder = RequestBuilder.get(
        RequestBuilder.resolveRequestUrl(getServiceUrl(), CommonConstants.URL_GET_DATA_PRODUCT_DRAFT, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.GET_DATA_PRODUCT_DRAFT);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
   ...
   *
   * @param deleteDataProductDraftOptions the
   *                                      {@link DeleteDataProductDraftOptions}
   *                                      containing the options for the call
   * @return a {@link ServiceCall} with a void result
   */
  public ServiceCall<Void> deleteDataProductDraft(DeleteDataProductDraftOptions deleteDataProductDraftOptions) {
    ...
    RequestBuilder builder = RequestBuilder.delete(RequestBuilder.resolveRequestUrl(getServiceUrl(),
        CommonConstants.URL_DELETE_DATA_PRODUCT_DRAFT, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.DLETE_DATA_PRODUCT_DRAFT);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    ResponseConverter<Void> responseConverter = ResponseConverterUtils.getVoid();
   ...
   * syntax.&lt;br/&gt;&lt;br/&gt;Supported patch operations
   * include:&lt;br/&gt;&lt;br/&gt;- Update the properties of a
   * data product&lt;br/&gt;&lt;br/&gt;- Add/Remove parts from a data product (up
   * to 20 parts)&lt;br/&gt;&lt;br/&gt;-
   * Add/Remove use cases from a data product&lt;br/&gt;&lt;br/&gt;- Update the
   * data product
   * state&lt;br/&gt;&lt;br/&gt;.
   *
   * @param updateDataProductDraftOptions the
   *                                      {@link UpdateDataProductDraftOptions}
   *                                      containing the options for the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link DataProductVersion}
   */
  public ServiceCall<DataProductVersion> updateDataProductDraft(
      UpdateDataProductDraftOptions updateDataProductDraftOptions) {
    ...
    RequestBuilder builder = RequestBuilder.patch(RequestBuilder.resolveRequestUrl(getServiceUrl(),
        CommonConstants.URL_UPDATE_DATA_PRODUCT_DRAFT, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.UPDATE_DATA_PRODUCT_DRAFT);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
    builder.bodyContent(com.ibm.cloud.sdk.core.util.GsonSingleton.getGsonWithoutPrettyPrinting()
        .toJson(updateDataProductDraftOptions.jsonPatchInstructions()), CommonConstants.CONTENT_TYPE_PATCH_JSON);
    ResponseConverter<DataProductVersion> responseConverter = ResponseConverterUtils
        .getValue(new com.google.gson.reflect.TypeToken<DataProductVersion>() {
        }.getType());
    return createServiceCall(builder.build(), responseConverter);
   ...
   * which can be used to download the
   * attachment. If a document does not have a completed attachment, the response
   * contains the `url` which was submitted
   * at document creation. If a document has an attachment that is incomplete, an
   * error is returned to prompt the user
   * to upload the document file and complete it.
   *
   * @param getDraftContractTermsDocumentOptions the
   *                                             {@link GetDraftContractTermsDocumentOptions}
   *                                             containing the options for the
   *                                             call
   * @return a {@link ServiceCall} with a result of type
   *         {@link ContractTermsDocument}
   */
  public ServiceCall<ContractTermsDocument> getDraftContractTermsDocument(
      GetDraftContractTermsDocumentOptions getDraftContractTermsDocumentOptions) {
    ...
    RequestBuilder builder = RequestBuilder.get(RequestBuilder.resolveRequestUrl(getServiceUrl(),
        CommonConstants.URL_GET_DRAFT_CONTRACT_TERMS_DOCUMENT, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.GET_DRAFT_CONTRACT_TERMS_DOCUMENT);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
   ...
   *
   * Contract documents can only be deleted for data product versions that are in
   * DRAFT state.
   *
   * @param deleteDraftContractTermsDocumentOptions the
   *                                                {@link DeleteDraftContractTermsDocumentOptions}
   *                                                containing the options for the
   *                                                call
   * @return a {@link ServiceCall} with a void result
   */
  public ServiceCall<Void> deleteDraftContractTermsDocument(
      DeleteDraftContractTermsDocumentOptions deleteDraftContractTermsDocumentOptions) {
    ...
    RequestBuilder builder = RequestBuilder.delete(RequestBuilder.resolveRequestUrl(getServiceUrl(),
        CommonConstants.URL_DELETE_DRAFT_CONTRACT_TERMS_DOCUMENT, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.DELETE_DRAFT_CONTRACT_TERMS_DOCUMENT);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    ResponseConverter<Void> responseConverter = ResponseConverterUtils.getVoid();
   ...
   *
   * Use this API to update the properties of a contract document that is
   ...
   * &lt;br/&gt;&lt;br/&gt;Contract terms documents can only be updated if the
   * associated data product version is in
   * DRAFT state.
   *
   * @param updateDraftContractTermsDocumentOptions the
   *                                                {@link UpdateDraftContractTermsDocumentOptions}
   *                                                containing the options for the
   *                                                call
   * @return a {@link ServiceCall} with a result of type
   *         {@link ContractTermsDocument}
   */
  public ServiceCall<ContractTermsDocument> updateDraftContractTermsDocument(
      UpdateDraftContractTermsDocumentOptions updateDraftContractTermsDocumentOptions) {
    ...
    RequestBuilder builder = RequestBuilder.patch(RequestBuilder.resolveRequestUrl(getServiceUrl(),
        CommonConstants.URL_UPDATE_DRAFT_CONTRACT_TERMS_DOCUMENT, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.UPDATE_DRAFT_CONTRACT_TERMS_DOCUMENT);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
    builder.bodyContent(com.ibm.cloud.sdk.core.util.GsonSingleton.getGsonWithoutPrettyPrinting().toJson(
        updateDraftContractTermsDocumentOptions.jsonPatchInstructions()), CommonConstants.CONTENT_TYPE_PATCH_JSON);
    ResponseConverter<ContractTermsDocument> responseConverter = ResponseConverterUtils
        .getValue(new com.google.gson.reflect.TypeToken<ContractTermsDocument>() {
        }.getType());
    return createServiceCall(builder.build(), responseConverter);
   ...
   *
   * @param publishDataProductDraftOptions the
   *                                       {@link PublishDataProductDraftOptions}
   *                                       containing the options for the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link DataProductVersion}
   */
  public ServiceCall<DataProductVersion> publishDataProductDraft(
      PublishDataProductDraftOptions publishDataProductDraftOptions) {
    ...
    RequestBuilder builder = RequestBuilder.post(RequestBuilder.resolveRequestUrl(getServiceUrl(),
        CommonConstants.URL_PUBLISH_DATA_PRODUCT_DRAFT, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.PUBLISH_DATA_PRODUCT_DRAFT);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
   ...
   *
   * @param getDataProductReleaseOptions the {@link GetDataProductReleaseOptions}
   *                                     containing the options for the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link DataProductVersion}
   */
  public ServiceCall<DataProductVersion> getDataProductRelease(
    ...
    RequestBuilder builder = RequestBuilder.get(
        RequestBuilder.resolveRequestUrl(getServiceUrl(), CommonConstants.URL_GET_DATA_PRODUCT_RELEASE, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.GET_DATA_PRODUCT_RELEASE);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
    if (getDataProductReleaseOptions.checkCallerApproval() != null) {
      builder.query(CommonConstants.CHECK_CALLER_APPROVER,
          String.valueOf(getDataProductReleaseOptions.checkCallerApproval()));
    }
    ResponseConverter<DataProductVersion> responseConverter = ResponseConverterUtils
        .getValue(new com.google.gson.reflect.TypeToken<DataProductVersion>() {
        }.getType());
   ...
   *
   * Use this API to update the properties of a data product release identified by
   * a valid
   * ID.&lt;br/&gt;&lt;br/&gt;Specify patch operations using http://jsonpatch.com/
   * syntax.&lt;br/&gt;&lt;br/&gt;Supported patch operations
   * include:&lt;br/&gt;&lt;br/&gt;- Update the properties of a
   ...
   *
   * @param updateDataProductReleaseOptions the
   *                                        {@link UpdateDataProductReleaseOptions}
   *                                        containing the options for the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link DataProductVersion}
   */
  public ServiceCall<DataProductVersion> updateDataProductRelease(
      UpdateDataProductReleaseOptions updateDataProductReleaseOptions) {
    ...
    RequestBuilder builder = RequestBuilder.patch(RequestBuilder.resolveRequestUrl(getServiceUrl(),
        CommonConstants.URL_UPDATE_DATA_PRODUCT_RELEASE, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.UPDATE_DATA_PRODUCT_RELEASE);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
    builder.bodyContent(com.ibm.cloud.sdk.core.util.GsonSingleton.getGsonWithoutPrettyPrinting()
        .toJson(updateDataProductReleaseOptions.jsonPatchInstructions()), CommonConstants.CONTENT_TYPE_PATCH_JSON);
    ResponseConverter<DataProductVersion> responseConverter = ResponseConverterUtils
        .getValue(new com.google.gson.reflect.TypeToken<DataProductVersion>() {
        }.getType());
    return createServiceCall(builder.build(), responseConverter);
   ...
   * to download the
   * attachment.&lt;br/&gt;&lt;br/&gt; If the document does not have an
   * attachment, the response contains the `url`
   * which was submitted at document creation.&lt;br/&gt;&lt;br/&gt; If the
   * document has an incomplete attachment, an
   * error is returned to prompt the user to upload the document file to complete
   * the attachment.
   *
   * @param getReleaseContractTermsDocumentOptions the
   *                                               {@link GetReleaseContractTermsDocumentOptions}
   *                                               containing the options for the
   *                                               call
   * @return a {@link ServiceCall} with a result of type
   *         {@link ContractTermsDocument}
   */
  public ServiceCall<ContractTermsDocument> getReleaseContractTermsDocument(
      GetReleaseContractTermsDocumentOptions getReleaseContractTermsDocumentOptions) {
    ...
    RequestBuilder builder = RequestBuilder.get(RequestBuilder.resolveRequestUrl(getServiceUrl(),
        CommonConstants.URL_GET_RELEASE_CONTRACT_TERMS_DOCUMENT, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.GET_RELEASE_CONTRACT_TERMS_DOCUMENT);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
   ...
   *
   * @param listDataProductReleasesOptions the
   *                                       {@link ListDataProductReleasesOptions}
   *                                       containing the options for the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link DataProductReleaseCollection}
   */
  public ServiceCall<DataProductReleaseCollection> listDataProductReleases(
    ...
    RequestBuilder builder = RequestBuilder.get(RequestBuilder.resolveRequestUrl(getServiceUrl(),
        CommonConstants.URL_LIST_DATA_PRODUCT_RELEASES, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.LIST_DATA_PRODUCT_RELEASES);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
    if (listDataProductReleasesOptions.assetContainerId() != null) {
      builder.query(CommonConstants.ASSET_CONTAINER_ID,
          String.valueOf(listDataProductReleasesOptions.assetContainerId()));
    }
    ...
    ResponseConverter<DataProductReleaseCollection> responseConverter = ResponseConverterUtils
        .getValue(new com.google.gson.reflect.TypeToken<DataProductReleaseCollection>() {
        }.getType());
    return createServiceCall(builder.build(), responseConverter);
  }
   ...
   *
   * @param retireDataProductReleaseOptions the
   *                                        {@link RetireDataProductReleaseOptions}
   *                                        containing the options for the call
   * @return a {@link ServiceCall} with a result of type
   *         {@link DataProductVersion}
   */
  public ServiceCall<DataProductVersion> retireDataProductRelease(
      RetireDataProductReleaseOptions retireDataProductReleaseOptions) {
    ...
    RequestBuilder builder = RequestBuilder.post(RequestBuilder.resolveRequestUrl(getServiceUrl(),
        CommonConstants.URL_RETIRE_DATA_PRODUCT_RELEASE, pathParamsMap));
    Map<String, String> sdkHeaders = SdkCommon.getSdkHeaders(CommonConstants.SERVICE_NAME,
        CommonConstants.SERVICE_VERSION, CommonConstants.RETIRE_DATA_PRODUCT_RELEASE);
    for (Entry<String, String> header : sdkHeaders.entrySet()) {
      builder.header(header.getKey(), header.getValue());
    }
    builder.header(HttpHeaders.ACCEPT, CommonConstants.CONTENT_TYPE_JSON);
